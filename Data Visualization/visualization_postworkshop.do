...
cd "/Users/SSB/Box Sync/D-lab/stata/Special Workshops/Visualization-Graphs/workshop apr062017"
...
#delimit ;
graph twoway (scatter change setting, msymbol(O) mcolor(green*.3));
#delimit cr
...
#delimit ;
graph twoway (scatter change setting, msymbol(O) mlcolor(green) mfcolor(yellow) )
(scatter effort setting, msymbol(O) mlcolor(green) mfcolor(orange) )
;
#delimit cr
...
graph twoway (scatter change setting, mlabel(country) mlabp(9) ) /* EDIT */ 
...
#delimit ;
coefplot model1, vertical  drop(_cons age married) xline(0)
          ytitle(" ""Regression Coefficient") 
		  xtitle(" " "Education Level")
		  xlabel(1 "Low" 
	          2 "Medium" 3 "High"  )
